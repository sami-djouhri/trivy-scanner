...
NTFY_URL="${NTFY_URL:-}"               # z.B. http://127.0.0.1:8147 (leer = ntfy aus)
NTFY_TOPIC="${NTFY_TOPIC:-homelab-security}"
NTFY_TOKEN_FILE="${NTFY_TOKEN_FILE:-}" # ntfy ist deny-all -> Publisher braucht Token (sec-publisher)
...
if [ -n "$NTFY_URL" ] && { [ "$crit_total" -gt 0 ] || [ "$high_total" -gt 0 ]; }; then
  auth=()
  [ -n "$NTFY_TOKEN_FILE" ] && [ -r "$NTFY_TOKEN_FILE" ] && auth=(-H "Authorization: Bearer $(cat "$NTFY_TOKEN_FILE")")
  curl -fsS -H "Title: Trivy CVE-Scan" "${auth[@]}" \
